...
import aiomultithreading as amt
from aiomultiprocess.core import get_context
from aiomultiprocess.pool import ProxyException, PoolResult
# from aiomultiprocess.pool import PoolWorker as ProcessWorker
from aiomultithreading import ProcessWorker
...
class PoolTest(TestCase):  # pylint: disable=too-many-public-methods
    ...
    #     worker = ProcessWorker(tx, rx, queuecount=1)
    #     worker.start()
    ...
    async def test_pool(self):
        ...
        async with amt.MultiPool(2, maxtasksperchild=5) as pool:
    ...
    async def test_pool_map(self):
        ...
        async with amt.MultiPool(2) as pool:
            obj = pool.map(mapper, values)
            self.assertIsInstance(obj, PoolResult)
            results = await obj
    ...
    async def test_pool_exception_handler(self):
        ...
        async with amt.MultiPool(2, exception_handler=handler) as pool:
            with self.assertRaises(ProxyException):
                await pool.apply(raise_fn, args=())
    ...
    def test_pool_args(self):
        with self.assertRaisesRegex(ValueError, "queue count must be <= process"):
            amt.MultiPool(4, queuecount=9)

    @async_test
    async def test_pool_closed(self):
        pool = amt.MultiPool(2)
        pool.close()
    ...
    async def test_pool_uvloop(self):
        try:
            ...
            async with amt.MultiPool(2, loop_initializer=uvloop.new_event_loop) as pool:
    ...
    async def test_pool_winloop(self):
        try:
            ...
            async with amt.MultiPool(2, loop_initializer=winloop.new_event_loop) as pool:
